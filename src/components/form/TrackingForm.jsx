...
const TrackingForm = ({ onSubmit, isLoading }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.firstDayOfLastMenstrualPeriod) {
      newErrors.firstDayOfLastMenstrualPeriod =
        "Please select your last menstrual period date";
    } else {
    ...
    if (!formData.preHeight || formData.preHeight <= 0) {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      onSubmit({
        userId: user.id,
        firstDayOfLastMenstrualPeriod: formatDateForApi(
          formData.firstDayOfLastMenstrualPeriod
        ),
      ...
      <div className="lmp-form-card">
        ...
        <form onSubmit={handleSubmit} className="lmp-form">
          ...
          <div className="form-group">
            ...
            />
            {errors.preWeight && (
              <span className="error-message">{errors.preWeight}</span>
            )}
          </div>
          <div className="form-group">
            ...
            <input
              ...
              className={errors.preHeight ? "error" : ""}
            />
            {errors.preHeight && (
              <span className="error-message">{errors.preHeight}</span>
            )}
          </div>

          <button type="submit" className="tracking-submit-btn" disabled={isLoading}>
            {isLoading ? (
              <>
