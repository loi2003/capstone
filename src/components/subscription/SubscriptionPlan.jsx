...
const SubscriptionPlan = () => {
      ...
      if (userId && userId !== 'null' && userId.trim() !== '') {
        try {
          ...
          const activeUserSub = Array.isArray(userSubData) 
            ...
            : userSubData && userSubData.status === 'Active' ? userSubData : null;
  ...
  const isUserCurrentPlan = (planId) => {
    // If user has a subscription, check if it matches this plan
    if (userSubscription && userSubscription.subscriptionPlanId) {
      return userSubscription.subscriptionPlanId === planId;
    }
    
    // If no user subscription, default to Free plan being active
    const freePlan = subscriptionPlans.find(plan => 
  ...
  const sortedPlans = [...subscriptionPlans].sort((a, b) => {
    const aIsFree = a.subscriptionName.toLowerCase() === 'free' || a.price === 0;
    const bIsFree = b.subscriptionName.toLowerCase() === 'free' || b.price === 0;
            ...
            return (
              <div
                ...
                className={`plan-card ${isCurrentPlan ? "current" : ""} ${
                  isPopular ? "popular" : ""
                } ${hoveredPlan === plan.id ? "hovered" : ""} plan-${planColor}`}
              ...
              >
                ...
                <button
                  className={`subscribe-btn ${
                    isCurrentPlan ? "current-btn" : ""
                  } ${isPopular ? "popular-btn" : ""}`}
