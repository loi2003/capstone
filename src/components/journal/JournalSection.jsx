...
import { getJournalByGrowthDataId, deleteJournal, getJournalDetail } from "../../apis/journal-api"
...
const JournalSection = ({ journalEntries, growthDataId, growthData, onError }) => {
  ...
  const handleViewDetails = async (journalId) => {
    try {
      ...
      if (response.data?.error === 0 && response.data?.data) {
        navigate("/pregnancy-tracking/journal-section/journal-detail", { state: { journal: response.data } })
      } else {
    ...
    return (
      <div className="journal-section">
        ...
        <div className="empty-state">
          ...
          <Link to={`/pregnancy-tracking/journal-section/journal-form?growthDataId=${growthDataId}`}>
          ...
          <div key={entry.id || Math.random()} className="journal-entry">
            <div className="entry-header">
              <div className="entry-info">
                <div className="week-badge">Week {entry.currentWeek || "N/A"}</div>
            ...
            <div className="entry-content">
              ...
              <div className="entry-metrics">
                {entry.currentWeight > 0 && (
                  <div className="metric">
                    <span className="metric-icon">
                      <img src={weightIcon} alt="Weight" className="journal-weight-icon" />
                    </span>
                ...
                {entry.mood && (
                  <div className="metric">
                    <span className="metric-icon">
                      <img src={moodIcon} alt="Mood" className="journal-mood-icon" />
                    </span>
                ...
                {entry.symptomNames?.length > 0 && (
                  <div className="entry-field">
                    <h4>Symptoms</h4>
                    <p className="entry-text">{entry.symptomNames.join(", ") || "No symptoms"}</p>
                  </div>
                )}
