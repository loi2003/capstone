      ...
      if (token) {
        try {
          ...
          if (userData && Number(userData.roleId) === 5) {
            console.log('User hợp lệ với roleId 5:', userData);
            setUser(userData);
          } else {
            console.warn('User không hợp lệ hoặc roleId không phải 5:', userData);
  ...
  return (
    <header className="clinic-header">
      <div className="container">
        <Link to="/clinic" className="logo" aria-label="Clinic Panel Home">
          <motion.svg
        ...
        <nav className={`clinic-nav ${isMenuOpen ? 'open' : ''}`} aria-label="Main navigation">
          <Link to="/clinic/dashboard" onClick={() => setIsMenuOpen(false)}>
            Dashboard
          </Link>
          <Link to="/clinic/schedule" onClick={() => setIsMenuOpen(false)}>
            Schedule
          </Link>
          <Link to="/clinic/patients" onClick={() => setIsMenuOpen(false)}>
            Patients
          </Link>
          <Link to="/clinic/support" onClick={() => setIsMenuOpen(false)}>
            Support
          </Link>
        </nav>
        <div className="clinic-user-section">
          {user ? (
            <div className="clinic-user-info">
              <motion.div
                className="clinic-user-icon"
              ...
              >
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    ...
                    fill="var(--clinic-text)"
                  />
                </svg>
              </motion.div>
              {isDropdownOpen && (
                <motion.div
                  className="clinic-user-dropdown"
                  initial={{ opacity: 0, y: -10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  transition={{ duration: 0.2 }}
                >
                  <span className="clinic-user-email">{user.email}</span>
                  <button onClick={handleLogout} className="clinic-logout-button">
                    Đăng Xuất
                  </button>
                </motion.div>
              )}
            ...
            <Link to="/signin" className="clinic-login-link">
              Đăng Nhập
            </Link>
          )}
        </div>
