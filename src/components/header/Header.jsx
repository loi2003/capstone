    ...
    const fetchUser = async () => {
      ...
      if (token) {
        try {
          ...
          if (userData && userData.roleId === 2) { // Chỉ hiển thị user icon cho roleId: 2
            setUser(userData);
            console.log('Thông tin người dùng:', userData);
          } else {
            console.warn('Người dùng không có roleId: 2 hoặc dữ liệu không hợp lệ:', userData);
            localStorage.removeItem('token'); // Xóa token nếu roleId không hợp lệ
            setUser(null);
          }
        } catch (error) {
          ...
          <div className="auth-section">
            {user ? (
              ...
              <div className="profile-section">
                <button className="profile-toggle" onClick={toggleDropdown} aria-label="User menu">
                ...
                {isDropdownOpen && (
                  <div className="profile-dropdown">
                    <span className="profile-email">{user.email || 'Người dùng'}</span>
