...
export const getAllJournals = async (token) => {
  try {
    ...
    console.error('Error fetching all journals:', error.response?.data || error.message);
    throw error;
  }
};
...
export const getJournalById = async (journalId, token) => {
  try {
    ...
    console.error('Error fetching journal by ID:', error.response?.data || error.message);
    throw error;
  }
};
export const getJournalDetail = async (journalId, token) => {
  try {
    ...
    console.error('Error fetching journal detail:', error.response?.data || error.message);
    throw error;
  }
};
...
export const getJournalByGrowthDataId = async (growthDataId, token) => {
  try {
    const response = await apiClient.get(`/api/journal/view-journal-by-growthdata-id/${growthDataId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
        Accept: 'text/plain',
      },
    ...
    console.error('Error fetching journal by growthDataId:', error.response?.data || error.message);
    throw error;
  }
};
...
export const createJournalEntry = async (journalData, token) => {
  try {
    ...
    journalData.SymptomNames?.forEach((symptom, i) => formData.append(`SymptomNames[${i}]`, symptom));
    journalData.RelatedImages?.forEach((img) => formData.append('RelatedImages', img));
    journalData.UltraSoundImages?.forEach((img) => formData.append('UltraSoundImages', img));

    const response = await apiClient.post('/api/journal/create-new-journal-entry', formData, {
      headers: {
...
export const editJournalEntry = async (journalData, token) => {
  try {
    ...
    const response = await apiClient.put('/api/journal/edit-journal-entry', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
        Authorization: `Bearer ${token}`,
        Accept: 'text/plain',
    ...
    console.error('Error editing journal entry:', error.response?.data || error.message);
    throw error;
  }
};
...
export const deleteJournal = async (journalId, token) => {
  try {
    const response = await apiClient.delete(`/api/journal/delete-journal?journalId=${journalId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
        Accept: 'text/plain',
      },
    ...
    console.error('Error deleting journal:', error.response?.data?.message || error.message);
    throw error;
  }
};
